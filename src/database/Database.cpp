...
Database::Database(const char* file_name) : database(nullptr){
    ...
    // If we are opening an in-memory database (during unit tests)
    if (std::string(file_name) == ":memory:") {
        
        int rc = sqlite3_open(file_name, &database);
        if (rc != SQLITE_OK) {
            std::string err_msg = "Failed to open in-memory DB: " + std::string(sqlite3_errmsg(database));
...
void Database::addToCart(int product_id, int user_id) {
    ...
    if(sqlite3_step(stmt_query.get_stmt()) == SQLITE_ROW){
        ...
        if(count > 0){
            ...
            std::cout << "Product count in Cart: " << count << ", Incrementing count to: " << count << std::endl;
            ...
            if (rc != SQLITE_DONE) {
                ...
                std::cout << "Count Increased Successfully" << std::endl;
            }
        }

        else{
            
            std::cout << "Product Not Yet in Cart, Adding to Table" << std::endl;
...
void Database::removeFromCart(int user_id, int product_id) {
    ...
    const char* sql_command =  "SELECT quantity " 
                            "FROM USER_CART uc "
                            "JOIN CART_ITEMS ci ON uc.CART_ID = ci.fk_cart_id "
                            "WHERE uc.fk_user_id = ? "
                            "AND ci.fk_product_id = ?";
    ...
    if(sqlite3_step(stmt_query.get_stmt()) == SQLITE_ROW){
        ...
        if(count >= 2){
            ...
            std::cout << "Product count in Cart: " << count << ", Decrementing count to: " << count - 1 << std::endl;
            ...
            if (rc != SQLITE_DONE) {
                ...
                std::cout << "Count Decreased Successfully" << std::endl;
            }
        }

        else{

            std::cout << "Product count in Cart: " << count << ", Deleting Item from Cart" << std::endl;
            ...
            if (rc != SQLITE_DONE) {
                ...
                std::cout << "Cart Item Deleted Successfully!" << std::endl;
            }
        }        
    }          
}
...
std::string Database::fetchPassword(std::string username){
    ...
    const char* sql_command = "SELECT password FROM USER_INFORMATION WHERE username = ?";
    PrepareStatement stmt_insert(sql_command, database);
    ...
    if(sqlite3_step(stmt_insert.get_stmt()) == SQLITE_ROW){
        ...
        const unsigned char* password_char = sqlite3_column_text(stmt_insert.get_stmt(), 0);
